...
def receive_file(s, file_path, host, port): # Hàm nhận file
...
def download_file(host, port, file_name, file_path): # Hàm tải file
    ...
    receive_file(s, file_path + file_name, host, port) # Gọi hàm nhận file
...
def main():
    ...
    ports = [clientConfig.port1, clientConfig.port2, clientConfig.port3, clientConfig.port4]
        ...
        with open('input.txt', 'r') as input_file: # Mở file ở chế độ đọc
            for line in input_file: # Đọc từng dòng trong file
                if line in processed_line: # Kiểm tra dòng đã được xử lí chưa
                    ...
                    threads = []
                    for port in ports:
                        t = threading.Thread(target=download_file, args=(host, port, file_name, file_path))
                        t.start()
                        threads.append(t)
